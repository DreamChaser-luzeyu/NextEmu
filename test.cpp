...
#include "modules/rtl_adapter/uart_encoder/include/UartEncoder.h"
...
//}
#include <chrono>
TEST_CASE(performance_test, "Performance test") {
    LOG_INFO("Hello test, good luck!");
    using namespace std::chrono;
    using std::chrono::steady_clock;


    auto *bus = new Base_ns::AddrBus();
    bus->setMem(0x80000000, 1024l * 1024l * 1024l * 4, nullptr);
    uint32_t data;
    auto last = steady_clock::now();
    for(long long i = 0; i < 65536; i++) {
        bus->load(0x80000000 + 4 * (i % 4), 4, (uint8_t*)&data);
    }
    ...
    Base_ns::SRAMAdapter* sramAdapter = new Base_ns::SRAMAdapter();
    sramAdapter->regAddr(&data1, 0, 4);
    sramAdapter->regAddr(&data2, 4, 4);
    sramAdapter->regAddr(&data3, 8, 4);
    sramAdapter->regAddr(&data4, 12, 4);
    bus->addDev(sramAdapter, 0x40000000);
    last = steady_clock::now();
    for(long long i = 0; i < 65536; i++) {
        bus->load(0x40000000 + 4 * (i % 4), 4, (uint8_t*)&data);
    }
    duration_ns = duration_cast<nanoseconds>(steady_clock::now() - last).count();
    LOG_INFO("%lu", duration_ns);
