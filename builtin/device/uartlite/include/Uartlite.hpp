...
public:
    ...
    Uartlite(size_t dev_size = 1024*1024) : waitACK(false), devSize(dev_size) {
        regs.rx_fifo = 0;
        regs.tx_fifo = 0;
...
//        LOG_INFO("Uartlite: Loading address 0x%08lx", begin_addr);
        ...
        if(begin_addr <= offsetof(UartliteReg_t, rx_fifo) && offsetof(UartliteReg_t, rx_fifo) <= begin_addr + len - 1) {
...
//        std::unique_lock<std::mutex> rx_locker(rxMutex);
        ...
        if (begin_addr <= offsetof(UartliteReg_t,tx_fifo) && offsetof(UartliteReg_t,tx_fifo) <= begin_addr + len - 1) {
            static_assert(offsetof(UartliteReg_t,tx_fifo) == 4);
            tx.push((char)(regs.tx_fifo));
        }
        // --- Check if accessed `regs.control`
        if (begin_addr <= offsetof(UartliteReg_t,control) && offsetof(UartliteReg_t,control) <= begin_addr + len - 1) {
