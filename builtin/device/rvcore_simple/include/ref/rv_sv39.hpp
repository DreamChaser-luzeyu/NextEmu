...
class rv_sv39 {
public:
    rv_sv39(Interface_ns::SlaveAtomicIO_I* bus):bus(bus){
        random = 0;
...
private:
    ...
    bool ptw(satp_def satp, uint64_t va_in, sv39_pte &pte_out, uint64_t &pagesize) {
        ...
        sv39_pte pte;
        for (int i=2;i>=0;i--) {
            int fb = bus->load(pt_addr+((i==2?va->vpn_2:(i==1?va->vpn_1:va->vpn_0))*sizeof(sv39_pte)),sizeof(sv39_pte),(uint8_t*)&pte);
            bool res = (fb == 0);
            if (!res) {
                // invalid pte address
                return false;
