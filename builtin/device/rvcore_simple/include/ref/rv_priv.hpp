...
class rv_priv {
public:
    rv_priv(uint64_t hart_id, uint64_t &pc, Interface_ns::SlaveAtomicIO_I* bus):hart_id(hart_id),cur_pc(pc),bus(bus),sv39(bus) {
        reset();
    }
    void reset() {
        trap_pc = 0;
    ...
    rv_exc_code va_if(uint64_t start_addr, uint64_t size, uint8_t *buffer, uint64_t &bad_va) {
        if (size == 4 && start_addr % 4 == 2) {
            ...
            if ( cur_priv == M_MODE || satp_reg->mode == 0) {
                bool pstatus = (bus->load(start_addr,size,buffer) == 0);
                ...
                bool pstatus = (bus->load(pa,size,buffer) == 0);
                if (!pstatus) return exc_instr_acc_fault;
                else return exc_custom_ok;
            }
        }
    ...
    rv_exc_code va_write(uint64_t start_addr, uint64_t size, const uint8_t *buffer) {
        ...
        if ( (cur_priv == M_MODE && (!mstatus->mprv || mstatus->mpp == M_MODE)) || satp_reg->mode == 0) {
            ...
            bool pstatus = (bus->store(start_addr,size,buffer) == 0);
            ...
            bool pstatus = (bus->store(pa,size,buffer) == 0);
    ...
    rv_exc_code va_lr(uint64_t start_addr, uint64_t size, uint8_t *buffer) {
        ...
        const csr_mstatus_def *mstatus = (csr_mstatus_def*)&status;
        if ( (cur_priv == M_MODE && (!mstatus->mprv || mstatus->mpp == M_MODE)) || satp_reg->mode == 0) {
            bus->reserve(start_addr,size,hart_id);
            bool pstatus = (bus->load(start_addr, size, buffer) == 0);
            if (!pstatus) return exc_store_acc_fault;
            else return exc_custom_ok;
        }
    ...
    rv_exc_code va_sc(uint64_t start_addr, uint64_t size, const uint8_t *buffer, bool &sc_fail) {
        ...
        const csr_mstatus_def *mstatus = (csr_mstatus_def*)&status;
        if ( (cur_priv == M_MODE && (!mstatus->mprv || mstatus->mpp == M_MODE)) || satp_reg->mode == 0) {
            sc_fail = !(bus->check(start_addr, size, hart_id));
            bool pstatus = (bus->store(start_addr,size,buffer) == 0);
            if (!pstatus) return exc_store_acc_fault;
            else return exc_custom_ok;
        }
            ...
            if (!mstatus->sum && priv == S_MODE && tlb_e->U) return exc_store_pgfault;
            uint64_t pa = tlb_e->ppa + (start_addr % ( (tlb_e->pagesize==1)?(1<<12):((tlb_e->pagesize==2)?(1<<21):(1<<30))));
            sc_fail = !(bus->check(pa, size, hart_id));
            bool pstatus = (bus->store(pa,size,buffer) == 0);
            if (!pstatus) return exc_store_pgfault;
            else return exc_custom_ok;
        }
    ...
    bool pa_amo_op(uint64_t pa, uint64_t size, amo_funct op, int64_t src, int64_t &dst) {
        ...
        if (size == 4) {
            int32_t res32;
            read_ok &= (bus->load(pa,size,(uint8_t*)&res32) == 0);
            res = res32;
        }
        else {
            read_ok &= (bus->load(pa,size,(uint8_t*)&res) == 0);
        }
        ...
        dst = res;
        return (bus->store(pa,size,(uint8_t*)&to_write) == 0);
    }
    ...
    rv_exc_code va_amo(uint64_t start_addr, uint64_t size, amo_funct op, int64_t src, int64_t &dst) {
        ...
        const csr_mstatus_def *mstatus = (csr_mstatus_def*)&status;
        if ( (cur_priv == M_MODE && (!mstatus->mprv || mstatus->mpp == M_MODE)) || satp_reg->mode == 0) {
            bool pstatus = pa_amo_op(start_addr,size,op,src,dst);
            if (!pstatus) return exc_store_acc_fault;
            else return exc_custom_ok;
        }
        else {
            ...
            if (!mstatus->sum && priv == S_MODE && tlb_e->U) return exc_store_pgfault;
            uint64_t pa = tlb_e->ppa + (start_addr % ( (tlb_e->pagesize==1)?(1<<12):((tlb_e->pagesize==2)?(1<<21):(1<<30))));
            bool pstatus = pa_amo_op(pa,size,op,src,dst);
            if (!pstatus) return exc_store_pgfault;
            else return exc_custom_ok;
        }
    }
...
private:
    ...
    rv_sv39<32> sv39;
    // pbus
    Interface_ns::SlaveAtomicIO_I* bus;
    // CSRs
    uint64_t        status;
    uint64_t        misa;
    uint64_t        medeleg;
