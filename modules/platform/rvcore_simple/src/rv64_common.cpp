...
bool RVCore_ns::RV64Core::memRead(uint64_t start_addr, uint64_t size, uint8_t *buffer) {
...
bool RVCore_ns::RV64Core::memWrite(uint64_t start_addr, uint64_t size, const uint8_t *buffer) {
    if (start_addr % size != 0) {
...
FuncReturnFeedback_e RVCore_ns::RV64Core::writeProgramCounter(RegItemVal_t reg_val)
{
    currentProgramCounter = reg_val.u64_val;
    return MEMU_OK;
}

FuncReturnFeedback_e RVCore_ns::RV64Core::setGPRByIndex(uint8_t gpr_index, int64_t val)
{
    assert(gpr_index < 32);
...
FuncReturnFeedback_e RVCore_ns::RV64Core::DumpProgramCounter_CoreAPI(RegisterItem_t &reg)
{
