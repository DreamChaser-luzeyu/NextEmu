...
MiniCPU_ns::VAddr_RW_Feedback_e MiniCPU_ns::RV64SV39_MMU::VAddr_ReadBuffer_MMU_API(uint64_t begin_addr, uint64_t size, uint8_t *buffer) {
    ...
    // --- Get & Check TLB Entry
    MiniCPU_ns::SV39_TLB_Entry_t* tlb_entry = getTLBEntry({.val = begin_addr});
...
MiniCPU_ns::VAddr_RW_Feedback_e MiniCPU_ns::RV64SV39_MMU::VAddr_InstFetch_MMU_API(uint64_t begin_addr, uint64_t size, void* buffer) {
    ...
    if(size == 4 && begin_addr % 4 == 2) {
        ...
        MiniCPU_ns::VAddr_RW_Feedback_e fb_1 = VAddr_InstFetch_MMU_API(begin_addr, 2, buffer);
        if(fb_1 != VADDR_ACCESS_OK) { return fb_1; }
        MiniCPU_ns::VAddr_RW_Feedback_e fb_2 = VAddr_InstFetch_MMU_API(begin_addr + 2, 2, ((uint8_t*)buffer) + 2);
...
MiniCPU_ns::VAddr_RW_Feedback_e MiniCPU_ns::RV64SV39_MMU::VAddr_WriteBuffer_MMU_API(uint64_t begin_addr, uint64_t size, const uint8_t *buffer) {
    const SATP_Reg_t* satp_reg = (SATP_Reg_t*)&satp;
    const CSReg_MStatus_t* mstatus = (CSReg_MStatus_t*)&status;
    // --- No need to do page-based vaddr converting
    if ( (currentPrivMode == M_MODE && (!mstatus->mprv || mstatus->mpp == M_MODE))
...
void MiniCPU_ns::RV64SV39_MMU::SV39_FlushTLB_sfence_vma(uint64_t vaddr, uint64_t asid) {
...
bool MiniCPU_ns::RV64SV39_MMU::isValidDirPTE(const SV39_PageTableEntry_t& pte) {
...
bool MiniCPU_ns::RV64SV39_MMU::isValidLeafPTE(const SV39_PageTableEntry_t &pte) {
    if(!pte.V) return false;                     // Current entry has to be VALID
...
uint16_t MiniCPU_ns::RV64SV39_MMU::getTLBIndexToSwapOut() {
    uint16_t index = lastSwappedIndex;
    lastSwappedIndex = (lastSwappedIndex + 1) % TLB_SIZE;
    return index;
}

MiniCPU_ns::SV39_TLB_Entry_t * MiniCPU_ns::RV64SV39_MMU::writeTLB(const SV39_PageTableEntry_t& leaf_pte, uint64_t page_size,
                                                               const SV39_VAddr_t& vaddr) {
    // --- Get the entry to swap out
    MiniCPU_ns::SV39_TLB_Entry_t* result = &(tlb[getTLBIndexToSwapOut()]);
...
MiniCPU_ns::SV39_TLB_Entry_t* MiniCPU_ns::RV64SV39_MMU::getTLBEntry(SV39_VAddr_t vaddr) {
    ...
    MiniCPU_ns::SV39_TLB_Entry_t* tlb_entry_ptr = nullptr;
//    // --- Use HashMap to speed up the TLB match procedure
//    if(tlbGetCache.find(vaddr.val & _4k_PAGE_ADDR_MASK) != tlbGetCache.end()) {
//        MiniCPU_ns::SV39_TLB_Entry_t* ptr = tlbGetCache.at(vaddr.val & _4k_PAGE_ADDR_MASK);
...
//        MiniCPU_ns::SV39_TLB_Entry_t* ptr = tlbGetCache.at(vaddr.val & _2M_PAGE_ADDR_MASK);
//        if((ptr->asid == satp.asid) || (ptr->G)) {
...
//        MiniCPU_ns::SV39_TLB_Entry_t* ptr = tlbGetCache.at(vaddr.val & _1G_PAGE_ADDR_MASK);
//        if((ptr->asid == satp.asid) || (ptr->G)) {
...
int MiniCPU_ns::RV64SV39_MMU::load(Interface_ns::addr_t begin_addr, uint64_t len, uint8_t *buffer) {
    return this->VAddr_ReadBuffer_MMU_API(begin_addr, len, buffer);
}

int MiniCPU_ns::RV64SV39_MMU::store(Interface_ns::addr_t begin_addr, uint64_t len, const uint8_t *buffer) {
